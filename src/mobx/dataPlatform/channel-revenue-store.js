...
class ChannelRevenueStore {
	...
	@observable storeChannelArr= [];
	@action
	getDownloadList(pageNo,pageSize,startDate,endDate,appId,adType,channelType,compareValue1,compareValue2,callback){
		let appStr=Client.returnArgument(appId);
		Client.getNullArgument('stat/getChannelDatasForDate?pageNo='+pageNo+'&pageSize='+pageSize+'&startDate='+startDate+'&endDate='+endDate+'&'+appStr+'&adType='+adType+'&channelType='+channelType).then(res=>{
			let key=[];
			let a1=[];
			let a2=[];
...
const channelRevenueStore = new ChannelRevenueStore();
export default channelRevenueStore
