...
@observer
export default class MediaIncomeAnalyze extends React.Component {
	constructor() {
		super()
		this.state = {
			size: 5,
	...
	componentWillMount() {
		...
		let { pageNo, pageSize, startValue, endValue, appId, os, adType, spaceId, formSizeId,compareValue1,compareValue2 } = this.state;
		let startStr = this.timeStr(startValue), endStr = this.timeStr(endValue);
		MediaIncomeAnalyzeStore.getDownloadList(pageNo, pageSize, startStr, endStr, appId, os, adType, spaceId, formSizeId,compareValue1,compareValue2,this.callback.bind(this))
	}
	timeStr(startValue){
		if (startValue) {
	...
	refreshList() {
		setTimeout(() => {
			let { pageNo, pageSize, startValue, endValue, appArr, osArr, typeArr, spaceArr, sizeArr,compareValue1,compareValue2 } = this.state;
			let startStr = this.timeStr(startValue), endStr = this.timeStr(endValue);
			let appId = this.arrStr(appArr);
			let os = this.arrStr(osArr);
			let adType = this.arrStr(typeArr);
			let spaceId = this.arrStr(spaceArr);
			let formSizeId = this.arrStr(sizeArr);
			MediaIncomeAnalyzeStore.getDownloadList(pageNo, pageSize, startStr, endStr, appId, os, adType, spaceId, formSizeId,compareValue1,compareValue2,this.callback.bind(this))
		}, 300)
	}
	echatsShow() {
		let { daysX ,a1,a2,isRate1,isRate2} = MediaIncomeAnalyzeStore
	...
	query() {
		...
		let { pageSize, startValue, endValue, appArr, osArr, typeArr, spaceArr, sizeArr,compareValue1,compareValue2 } = this.state;
		let startStr = this.timeStr(startValue), endStr = this.timeStr(endValue);
		let appId = this.arrStr(appArr);
		let os = this.arrStr(osArr);
		let adType = this.arrStr(typeArr);
		let spaceId = this.arrStr(spaceArr);
		let formSizeId = this.arrStr(sizeArr);
		let arr = [appId, os, adType, spaceId, formSizeId];
		let num = 0;
		for (let i = 0; i < arr.length; i++) {
			if (arr[i] == '') {
				num += 1;
		...
		this.setState({
			geziWidth: 100 / (23 - num) + '%'
		})
		MediaIncomeAnalyzeStore.getDownloadList(1, pageSize, startStr, endStr, appId, os, adType, spaceId, formSizeId,compareValue1,compareValue2,this.callback.bind(this))
		MediaIncomeAnalyzeStore.storeTypeArr = this.state.typeArr;
		MediaIncomeAnalyzeStore.storeSpaceArr = this.state.spaceArr;
		MediaIncomeAnalyzeStore.storeSizeArr = this.state.sizeArr;
		MediaIncomeAnalyzeStore.storeOsArr = this.state.osArr;
		MediaIncomeAnalyzeStore.storeAppArr = this.state.appArr;
	}
	componentWillUnmount() {
		MediaIncomeAnalyzeStore.storeTypeArr = [];
		MediaIncomeAnalyzeStore.storeSpaceArr = [];
		MediaIncomeAnalyzeStore.storeSizeArr = [];
		MediaIncomeAnalyzeStore.storeOsArr = [];
		MediaIncomeAnalyzeStore.storeAppArr = [];
	}
	onCompareChange1(e) {
		this.setState({
			compareValue1: e
	...
	render() {
		let { appArr, oursSpace, adSizes } = GeneralizeManageStore;
				...
				<div className="content">
					<div className="contentBulk3" style={{ paddingTop: 10 }}>
						...
						<div className="listROwlet2" style={{ minHeight: 60, lineHeight: '30px' }}>
							...
							<div className="form-right-multiple" style={{ width: '50%' }}>
								<Checkbox onChange={this.onSizeAllChange.bind(this)}
										  indeterminate={this.state.sizeArr.length!=0&&this.state.sizeArr.length<adSizes.length}
									checked={this.state.sizeArr.length==adSizes.length} value={this.state.sizeArr.length==adSizes.length} />
								<Select
									mode="multiple"
									placeholder="请选择广告尺寸"
									value={this.state.sizeArr}
									maxTagCount={1}
									onChange={this.onSizeChange.bind(this)}
									style={{ width: '100%', marginLeft: 10 }}
								>
									{adSizes.map((i, k) => (
										<Option value={i.id} key={k}>{i.width}×{i.height}</Option>
									))}
								</Select>
							</div>
						</div>
