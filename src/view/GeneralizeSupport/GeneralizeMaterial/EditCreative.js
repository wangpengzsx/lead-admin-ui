...
@observer
export default class EditCreative extends React.Component {
	constructor(){
		super()
		this.state={
			value:'BANNER',
	...
	componentWillMount(){
		let that=this;
		Client.getleadArr('creative/findAllById',{id:localStorage.getItem('editCreative')}).then(res=>{
			if(res.clickFallbackURL&&res.clickFallbackURL!=''){
				that.setState({
					value:res.adForm.adType,
					name:res.name,
					clickTargetURL:res.clickFallbackURL,
					deeplink:res.clickTargetURL,
					clickAction:res.clickAction,
					fields:res.fields
				})
			}else{
				that.setState({
					value:res.adForm.adType,
					name:res.name,
					clickTargetURL:res.clickTargetURL,
					clickAction:res.clickAction,
					fields:res.fields
				})
			}
			UploadOrginality.fields=res.fields;
			UploadOrginality.getleadAdFormsA(res.adForm.adType);
			UploadOrginality.formId=res.adForm.id;
			UploadOrginality.getSizeArrA(res.adForm.id,res.fields);
			UploadOrginality.sizeId=res.adSize.id;
			UploadOrginality.getAdSpace({adFormId:res.adForm.id,spaceSizeId:res.adSize.width+'*'+res.adSize.height})
		})
	}
	...
	confirm(){
		...
		if(this.state.fields.length==fields.length&&this.state.clickTargetURL!=''&&this.state.clickAction!=''&&this.state.name!='') {
			...
			if(this.state.deeplink==''){
				data={
					id: localStorage.getItem('editCreative'),
				...
				<div className="content">
					...
					<div className="contentBulk">
							...
							<div className="form-right">
								...
								{this.state.nameError?(
									<HintAlert left={360} width={170}
											   message={this.state.name==''?"创意名称不能为空":"创意名称不能大于40个"}/>
								):null}
						...
						<div className="accountListRow">
							...
							<div className="form-right">
								<RadioGroup onChange={this.onClickChange.bind(this)} value={this.state.clickAction}>
