...
@observer
export default class NewAdSkylight extends React.Component {
	constructor(props) {
		super(props)
		this.state = {
			value: '',
	...
	componentWillMount() {
		SkylightMangeStore.getFiled(this.props.location.formId);
		// SkylightMangeStore.getFileds(this.props.location.formId);
		//SkylightMangeStore.getSizeArr(this.props.location.id);
			...
			if (this.props.location.spaceId) {
				...
				data = {
					...
					creative: {
						...
						fields: this.state.fields,
						adForm: { id: this.props.location.formId },
						sizeId: { id: this.props.location.sizeId == "undefined" ? "undefined" : "" },
						creativeState: "ONLINE"
					},
					//adSpace: { id: this.props.location.spaceId },
					appGroup: { id: this.props.location.appgroupId },
					adForm: { id: this.props.location.formId },
					sizeId: { id: this.props.location.sizeId == "undefined" ? "undefined" : "" },
					clickAction: this.props.location.clickAction == "undefined" ? "undefined" : "",
					adType: this.props.location.adType
				}
			}
			SkylightMangeStore.creatingSky(data, this.callback.bind(this))
		} else {
	...
	aaa(i) {
		...
		if (i.type = "VIDEO") {
			return '视频尺寸' + i.constraints[0].value + '×' + i.constraints[1].value + ',' + '大小不超过' + i.constraints[2].value;
		}
	}
				...
				<div className="content">
					...
					<div className='contentBulk' style={{ position: 'relative', height: 400, margin: 0 }}>
									...
									i.type == "IMAGE" ? (
										<div className="form-right">
											...
											<input className="fileButton" type="file" title="" style={{ width: 250, height: 30 }}
												ref={i.name}
												onChange={() => Client.imgFile + this.onFileChange(i.name)} />
											<button>
												上传图片
												</button>
						...
						<div className="accountListRow">
							...
							<div className="form-right">
								<input type="text" className={this.state.urWlError ? "borderError" : "border1"}
									style={{ width: 360, height: 30 }}
