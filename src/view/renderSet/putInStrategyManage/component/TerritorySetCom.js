...
import area from "../../../../common/area3";
export default class TerritorySetCom extends React.Component {
	constructor(){
		super();
		this.state={
			area: area,
	...
	componentWillMount(){
		localStorage.setItem('initChoiceArr',JSON.stringify(localStorage.getItem('choiceArr')?JSON.parse(localStorage.getItem('choiceArr')):[]));
		localStorage.setItem('initExcludeArr',JSON.stringify(localStorage.getItem('excludeArr')?JSON.parse(localStorage.getItem('excludeArr')):[]));
	...
	confirm(){
		localStorage.setItem('choiceArr',JSON.stringify(this.state.choiceArr));
		localStorage.setItem('excludeArr',JSON.stringify(this.state.excludeArr));
		this.props.history.push({pathname:'/AddPutInStrategy'})
	}
	cancel(){
		localStorage.setItem('choiceArr',localStorage.getItem('initChoiceArr'));
		localStorage.setItem('excludeArr',localStorage.getItem('initExcludeArr'));
		this.props.history.push({pathname:'/AddPutInStrategy'})
		localStorage.removeItem('initChoiceArr');
		localStorage.removeItem('initExcludeArr');
	}
	render() {
		return (
			<div>
